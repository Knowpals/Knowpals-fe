...
function WrongQuestionItem({ question, index }) {
  ...
  const diffGradientOption = v?.difficulty_gradient ? {
    ...
    legend: { data: ['你的正确率', '班级平均'], bottom: 0, textStyle: { fontSize: 10 } },
    ...
    series: [
      { name: '你的正确率', type: 'bar', data: v.difficulty_gradient.student_rates.map(r => Math.round(r * 100)),
    ...
    <div style={{ marginBottom: 10, border: '1px solid #f0f0f0', borderRadius: 10, overflow: 'hidden' }}>
      ...
      <div onClick={() => setOpen(!open)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: open ? '#f8f5ff' : '#fff', cursor: 'pointer', transition: 'background 0.15s' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 2, fontSize: 11 }}>
            ...
            <span style={{ color: '#ef4444' }}>答：{formatAnsDisplay(question.studentAnswer)}</span>
            <span style={{ color: '#10b981' }}>正解：{formatAnsDisplay(question.correctAnswer)}</span>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginLeft: 8 }}>
          <span style={{ padding: '3px 10px', borderRadius: 12, background: lc + '18', color: lc, fontSize: 11, fontWeight: 600 }}>{question.primaryLevel} · {question.primaryError}</span>
      ...
      {open && (
        <div style={{ padding: '12px 14px 14px', borderTop: '1px solid #f0f0f0', background: '#fafbfc' }}>
          ...
          {/* ====== 六维可视化 ====== */}
          <div className="la-viz-grid-v4">

            {/* ① 难度梯度条形图 */}
            {v?.difficulty_gradient && (
              <div className="la-viz-card-v4">
                <div className="la-viz-card-title-v4"><span className="la-dot" style={{ background: '#7c3aed' }} />① 难度梯度</div>
            ...
            {v?.time_comparison && (
              <div className="la-viz-card-v4">
                <div className="la-viz-card-title-v4"><span className="la-dot" style={{ background: '#f59e0b' }} />② 答题时间对比</div>
                <div className="la-time-compare-v4">
                ...
                <div className="la-time-bar-wrap-v4">
                  <div className="la-time-class-range-v4" style={{ left: `${(v.time_comparison.class_p25 / Math.max(v.time_comparison.class_p75 * 1.3, v.time_comparison.student_time * 1.1)) * 100}%`, right: `${100 - (v.time_comparison.class_p75 / Math.max(v.time_comparison.class_p75 * 1.3, v.time_comparison.student_time * 1.1)) * 100}%` }}>班级 25%-75%</div>
                  <div className="la-time-student-marker-v4" style={{ left: `${(v.time_comparison.student_time / Math.max(v.time_comparison.class_p75 * 1.3, v.time_comparison.student_time * 1.1)) * 100}%` }} />
                </div>
                <div className="la-time-legend-v4">
                  <span>⬤ 班级区间 P25–P75</span>
                  <span style={{ color: '#ef4444' }}>▲ 你的用时</span>
            ...
            {v?.error_option_analysis && (
              <div className="la-viz-card-v4">
                <div className="la-viz-card-title-v4"><span className="la-dot" style={{ background: '#ef4444' }} />③ 错误选项分析</div>
                ...
                {v.error_option_analysis.error_tags.map((et, ti) => (
                  <span key={ti} className="la-error-tag-v4" style={{ background: '#fee2e2', color: '#dc2626' }}>
                    {et.tag} <b>({Math.round(et.freq * 100)}%)</b>
                  </span>
                ))}
                <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 6 }}>百分比 = 该标签在学生历史错误中的出现频率</div>
              </div>
            )}
            ...
            {v?.solution_steps && (
              <div className="la-viz-card-v4">
                <div className="la-viz-card-title-v4"><span className="la-dot" style={{ background: '#8b5cf6' }} />④ 解题步骤定位</div>
                {v.solution_steps.map((st, si) => (
                  <div key={si} className="la-step-row-v4">
                    <div className={`la-step-num-v4 ${st.ok === true ? 'ok' : st.ok === false ? 'err' : 'skp'}`}>{st.step}</div>
                    <div className="la-step-compare-v4">
                      <div style={{ fontSize: 11, fontWeight: 500, color: '#333' }}>{st.desc}</div>
                      <div className="la-step-std-v4">✓ 标准：{st.standard}</div>
                      {st.ok === false && <div className="la-step-stu-v4">✗ 你：{st.student}</div>}
                      {st.ok === true && <div style={{ color: '#10b981', fontSize: 10 }}>✓ 你：{st.student}</div>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {v?.history_trend && (
              <div className="la-viz-card-v4">
                <div className="la-viz-card-title-v4"><span className="la-dot" style={{ background: '#10b981' }} />⑤ 历史趋势 — {v.history_trend.knowledge_point}</div>
            ...
            {v?.prerequisite_gap && (
              <div className="la-viz-card-v4">
                <div className="la-viz-card-title-v4"><span className="la-dot" style={{ background: '#f97316' }} />⑥ 前置知识掌握缺口</div>
          ...
          {/* ====== 综合结论面板 ====== */}
          {v?.conclusion && (
            <div className="la-conclusion-v4">
              <div className="la-conclusion-header-v4">
                <span className="la-conclusion-tag-v4" style={{ background: lc }}>
                  {v.conclusion.primary_category}
                </span>
                <span className="la-conclusion-confidence-v4">
              ...
              <div className="la-conclusion-bar-v4">
                <div className="la-conclusion-bar-fill-v4" style={{ width: `${Math.round(v.conclusion.confidence * 100)}%`, background: lc }} />
              </div>
              <div className="la-conclusion-summary-v4">
                <b>📋 证据摘要：</b>{v.conclusion.evidence_summary}
              </div>
              <div className="la-conclusion-advice-v4">

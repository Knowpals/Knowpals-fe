...
// 学习 tab = V1 class-detail.html 的班级列表逻辑
const StudentLearn = () => {
  ...
  return (
    <StudentLayout title="学习">
      {/* 顶部统计 - V1 class-detail stats-section */}
      ...
      {/* 未完成预习通知 - V1 bottom-notice */}
      {totalPending > 0 && (
        <div className="bottom-notice-v1" onClick={() => {
          // 跳到有未完成任务的第一个班级
          for (const cls of classes) {
            const cid = cls.class_id || cls.id;
            if (pendingCounts[cid] > 0) {
              navigate(`/student/class/${cid}?name=${encodeURIComponent(cls.class_name || cls.name || '')}`);
              return;
            }
          }
        }}>
      ...
      <div className="classes-list-v1">
        ...
        {classes.length === 0 ? (
          <Empty description="还没有加入任何班级" image={Empty.PRESENTED_IMAGE_SIMPLE}>
            <Button type="primary" onClick={() => setJoinModal(true)}>输入邀请码加入班级</Button>
          </Empty>
        ) : (
          classes.map((cls) => {
            const classId = cls.class_id || cls.id;
      ...
      {joinModal && (
        <div className="modal-overlay" onClick={() => setJoinModal(false)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <input className="modal-input" placeholder="请输入邀请码" value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} />
